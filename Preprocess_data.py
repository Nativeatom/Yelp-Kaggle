...
print("SVM")
clf = SVC()
clf.fit(review_train, star_train) 
...
# Validation data
review_validation_result = clf.predict(review_test)
misClass = 0
for i in range(0,len(review_validation_result)):
    if review_validation_result[i]!=star_test[i]:
        misClass = misClass + 1
accurarcy = 1 - (misClass/len(star_test))
l1accurarcy = sum(abs(review_validation_result-star_test))
print("Acurarcy:",accurarcy)
print("L1 Accurarcy: ",l1accurarcy/len(star_test))
